...
void Suggest::getSuggestions(ProximityInfo *pInfo, void *traverseSession,
    ...
    // Gesture policy is null when libjni_latinimegoogle is absent; return empty so the
    // Java-side fallback (gesturePathToLetters) can handle the batch input instead.
    if (!TRAVERSAL) {
        return;
    }
    PROF_INIT;
